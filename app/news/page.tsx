    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-slate-400 text-sm">
          {items.length} stories from 15+ sources · Updated every 6 hours
        </p>
      </div>
      ...
      {items.length === 0 ? (
        ...
        <NewsFilters items={items} categories={categories} />
      )}
    </div>
  );
}
  ...
  return (
    <div className="rounded-lg border border-dashed border-slate-800 bg-slate-900/50 p-12 text-center">
      ...
      <p className="text-sm text-slate-600 max-w-sm mx-auto">
        News items populate automatically after the GitHub Actions workflow runs.
      </p>
    </div>
  );
}
