...
export default function NewsCard({ item }: { item: NewsItem }) {
  ...
  const summary = item.summary.replace(/\s*\[…\]\s*$/, '').replace(/\s*\.\.\.\s*$/, '').trim();

  return (
    <a href={item.source_url} target="_blank" rel="noopener noreferrer" className="group block">
      <div className="rounded-lg border border-slate-800 bg-slate-900 p-4 transition-all hover:border-blue-900/60 hover:bg-slate-800/80 h-full flex flex-col">
